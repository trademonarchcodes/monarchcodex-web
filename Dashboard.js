...
function renderProfile() {
    ...
    /* REFERRAL CODE + LINK - FIX LOADING ISSUE */
    try {
        const referralCodeEl = getElement("referralCode");
        const referralLinkEl = getElement("referralLink");
        if (referralCodeEl) {
            // Generate code from uid or user id
            let code = "";
            if (currentProfile && currentProfile.uid) {
                code = String(currentProfile.uid).toUpperCase().substring(0,8);
            } else if (currentProfile && currentProfile.referral_code) {
                code = String(currentProfile.referral_code).toUpperCase();
            } else {
                code = currentUser.id.substring(0,6).toUpperCase();
            }
            if (!code || code === "LOADING...") code = "MC" + currentUser.id.substring(0,4).toUpperCase();
        ...
        if (copyCodeBtn && !copyCodeBtn.dataset.bound) {
            copyCodeBtn.dataset.bound = "1";
        ...
        if (copyLinkBtn && !copyLinkBtn.dataset.bound) {
            ...
            copyLinkBtn.addEventListener("click", function() {
                ...
                if (linkEl) {
                    navigator.clipboard.writeText(linkEl.value.trim()).then(()=>{
    ...
    } catch(e) { console.error("Referral code error", e); }

    /* BANK COPY BUTTONS */
    try {
        document.querySelectorAll('#bankPayment .copy-btn[data-copy]').forEach(btn => {
            ...
            btn.onclick = () => {
                ...
                if (el) {
                    ...
                    const old = btn.textContent; btn.textContent = "Copied!"; btn.style.background = "#00c851"; btn.style.borderColor="#00c851"; btn.style.color="#fff";
                    setTimeout(()=> { btn.textContent = old; btn.style.background=""; btn.style.borderColor=""; btn.style.color=""; }, 1500);
                }
            };
        });
    } catch(e){}
