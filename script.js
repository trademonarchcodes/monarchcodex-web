...
async function loadWithdrawalData() {
    try {
        ...
        if (investmentError) {
            console.error("Withdrawal investment error:", investmentError);
            return;
        }
        ...
        if (earningsError) {
            console.error("Withdrawal earnings error:", earningsError);
            return;
        }
        ...
        /* CALCULATE ALREADY REQUESTED/WITHDRAWN */
        let withdrawnProfit = 0;
        let withdrawnCapital = 0;
        ...
        (withdrawals || []).forEach(withdrawal => {
            ...
            if (
                ...
                withdrawnProfit += Number(withdrawal.amount) || 0;
            }
            ...
            if (
                ...
                withdrawnCapital += Number(withdrawal.amount) || 0;
            }
        ...
        /* UPDATE BALANCES */
        const profitElement =
            document.getElementById("withdrawableProfit");
        ...
        const capitalElement =
            document.getElementById("withdrawableCapital");

        if (capitalElement) {
            capitalElement.textContent =
                formatMoney(totalWithdrawableCapital);
        }

        const lockedElement =
            document.getElementById("lockedCapital");
...
async function submitWithdrawalRequest() {
    ...
    const typeInput =
        document.getElementById("withdrawalType");

    const bankInput =
        document.getElementById("withdrawalBankName");
